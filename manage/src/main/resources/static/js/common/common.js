...
var Ms = MS =  {
	...
	saveLoading: function saveLoading() {
        layer.load(2, {shade : 0.01, time : 50000});
    },
    closeLoading: function closeLoading() {
        parent.layer.closeAll('loading');
    },
    clearGridSelections: function clearGridSelections(tableId) {
        $('#' + tableId).datagrid('clearSelections');
    },
	reloadDataGrid: function reloadDataGrid(tableId) {
        $('#' + tableId).datagrid('reload');
    },
	reloadParentDataGrid: function reloadParentDataGrid(tableId) {
    ...
    /*只能为数字且只能保留5为小数 */
	onlyFive: function onlyFive(obj) {
		obj.value = obj.value.replace(/[^\d.]/g, "");
		obj.value = obj.value.replace(/^\./g, "");
		obj.value = obj.value.replace(/\.{2,}/g, ".");
		obj.value = obj.value.replace(".", "$#$").replace(/\./g, "").replace("$#$", ".");
	...
	/* 验证字符长度 str当前元素 num1和num2为字符长度范围 */
    checkStrLenght : function checkStrLenght(str,num1,num2){
        if(str.val().length >= num1 && str.val().length <= num2){
            return true;
        }else{
            return false;
        }
...
//		return (new RegExp(/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,20}$/).test(str));
	...
	/* 不能输入中文 */
    noChinese : function noChinese(obj) {
        obj.value = obj.value.replace(/^[\u4e00-\u9fa5]{0,}$/,'');
    },
	/* 不能输入英文 */
    onlyChinese : function onlyChinese(obj) {
        obj.value = obj.value.replace(/^[^\u4e00-\u9fa5]{0,}$/,'');
    },

    /* 允许首位输入0 */
    allowFirstZero : function onlyNumber(obj){
        obj.value = obj.value.replace(/\D|^/g,'');
    },
    ...
    /* 校验整数 */
    isInteger : function onlyNumber2(val){
        var reg =/^-?\d+$/;
        return reg.test(val);
    },

    /* 校验ip */
    isValidIP : function onlyNumber(ip){
