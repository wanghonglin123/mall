...
$(function(){
    ...
    if($('.verification-code a').attr('data-cookie')){
        var cookie_nave = $('.verification-code a').attr('data-cookie');
        if(YT.getCookie(cookie_nave+'cookietime')){
            YT.codeCountDown($('.verification-code a'));
        }
        $('.verification-code a').click(function () {
            doLogin(2,cookie_nave);
        });
    }
    /* 点击注册事件 */
...
/** 判斷登錄及獲取驗證碼，type為1表示登錄，為2表示獲取驗證碼,cookie_nave驗證碼的cookie名稱 */
function doLogin(type,cookie_nave) {
    if($('#_captcha_img_content').is(':hidden')){
    ...
    if(type ==2){
        if(!YT.getCookie(cookie_nave+'cookietime')){
            $.post('/sys/sendSmsCode-login',param,function (ret) {
                if(ret.status == 200){
                    layer.msg('验证码已经发送到你手机，请注意查收', {icon:1});
                    YT.codeCountDown($('.verification-code a'),function () {
...
function checkEmail(){
    ...
    if(strEmail == ""){
    	...
    	if(strEmail.toString().length < 6 || strEmail.toString().length > 30){
    		$("#hint8").html(" * 电子邮箱长度为6-30字符，请重新输入！");
    		$("#email").addClass("border-red").removeClass("border-gray");
            return false;
    	}else{
    	    if(strEmail == pwd){
